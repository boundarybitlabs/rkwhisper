...
impl PyClientHello {
    #[new]
    #[pyo3(signature = (model, mode="batch".to_string(), lang="en".to_string(), task="transcribe".to_string(), max_new_tokens=128, beam_size=5, notimestamps=false, suppress_tokens="default".to_string(), audio_format=None, vad=None))]
    fn new(
        model: String,
        mode: String,
        lang: String,
...
impl SyncSession {
    ...
    fn recv_response(&mut self) -> PyResult<Option<String>> {
        // Simple placeholder: return text if it's a segment
        match self.inner.recv_response() {
            Ok(rkwhisper_protocol::Response::Segment { text, .. }) => Ok(Some(text)),
            Ok(rkwhisper_protocol::Response::Done { .. }) => Ok(None),
            Ok(rkwhisper_protocol::Response::Error { error }) => Err(PyRuntimeError::new_err(error)),
            Ok(_) => Ok(Some("other response".to_string())),
