use rkwhisper_client::{asynchronous::Session, ClientHello};
use std::env;

#[tokio::main]
async fn main() -> anyhow::Result<()> {
    let socket_path = env::var("RKWHISPER_SOCKET").unwrap_or_else(|_| "/run/rkwhisper/asr.sock".to_string());
    let model = env::var("RKWHISPER_TEST_MODEL").unwrap_or_else(|_| "whisper-small-30s".to_string());
